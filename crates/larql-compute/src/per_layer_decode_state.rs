...
mod tests {
    ...
    fn is_complete_for_pins_per_layer_count() {
        ...
        s.h_in_per_layer.push(CpuStateHandle::boxed(Array2::zeros((1, 4))));
        s.k_new_per_layer.push(CpuStateHandle::boxed(Array2::zeros((1, 2))));
        s.v_new_per_layer.push(CpuStateHandle::boxed(Array2::zeros((1, 2))));
    ...
    fn reset_clears_without_freeing_capacity() {
        ...
        for _ in 0..4 {
            s.h_in_per_layer.push(CpuStateHandle::boxed(Array2::zeros((1, 8))));
        }
        s.reset();
        assert!(s.h_in_per_layer.is_empty());
